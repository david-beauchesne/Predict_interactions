...
where $w_t$ the weight (in $[0;1]$). For our analyses, the first element on the right-hand side of \eqref{eq2} is the similarity between the sets of resources (or consumers) for two taxa. The second is the Tanimoto similarity used to evaluate taxonomic similarity of the same taxa. When $w_t = 0$ only resource or consumer sets are used to compute similarity, while $w_t = 1$ solely uses taxonomy. This approach to consider the relative contribution of two sets of vectors to the Tanimoto similarity was developed by \citet{Desjardins-Proulx}.
